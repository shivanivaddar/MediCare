...
function App() {
  ...
  return (
    <main className="app-shell">
      ...
      <section className="booking-panel"><div className="section-heading"><div><p className="eyebrow">Your order</p><h2>Medicine booking</h2></div><span className="result-count">{booking.reduce((total, item) => total + item.quantity, 0)} items</span></div>{booking.length ? <><div className="booking-list">{booking.map((item) => <div className="booking-row" key={item._id}><img src={item.image || starterMedicines[0].image} alt="" /><div><strong>{item.name}</strong><small>{item.shop || 'MediCare shop'}</small></div><div className="quantity-controls"><button onClick={() => updateBookingQuantity(item._id, -1)}>-</button><span>{item.quantity}</span><button onClick={() => updateBookingQuantity(item._id, 1)}>+</button></div><b>₹{item.price * item.quantity}</b></div>)}</div><div className="booking-total"><span>Total</span><strong>₹{bookingTotal}</strong><button className="primary-button" onClick={() => setMessage('Booking saved. Checkout will be available in the next step.')}>Book medicines</button></div></> : <p className="empty-booking">Your booking is empty. Choose a medicine above to begin.</p>}</section>
